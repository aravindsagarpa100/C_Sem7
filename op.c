...
void make_parsing_table()
{
    ...
    { 
        for(j = 2;j < strlen(productions[i]) - 1;j++)
        {
            ...
            {
                for(k = 0;k < strlen(leading_list[get_non_term(productions[i][j + 1])]);k++)
                {
            ...
            {
                for(k = 0;k < strlen(trailing_list[get_non_term(productions[i][j])]);k++)
                {
    ...
    for(k = 0;k < strlen(leading_list[0]);k++)  //handling $
    {
...
void main()
{
    ...
    mod_input_string[0] = '$';
    for(i = 0,t = 1;i < strlen(input_string) - 1;)
    {   
